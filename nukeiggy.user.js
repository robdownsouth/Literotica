...
//
// @include 	http://forum.literotica.com/forumdisplay.*
// @include		http://forum.literotica.com/showthread.php?*
// @include		http://forum.literotica.com/profile.php?do=editlist
// @include		http://forum.literotica.com/profile.php?do=addlist&userlist=ignore*
// @include 	https://forum.literotica.com/forumdisplay.*
// @include		https://forum.literotica.com/showthread.php?*
// @include		https://forum.literotica.com/profile.php?do=editlist
// @include		https://forum.literotica.com/profile.php?do=addlist&userlist=ignore*
// ==/UserScript==
...
(function() {
	...
	var litbi_ignorelist = new Array;
	var litbi_ignoredposts = new Array;
	...
	if(window.location.pathname == "/profile.php") {
		...
		for(var i=0; i<xpathResult.snapshotLength; i++) {
			...
			if(rx = r.exec(thisItem.id)) {
				litbi_ignorelist.push(rx[1]);
			}
		}
		localStorage.setItem("litbi_ignorelist", litbi_ignorelist.join());
		GM_log(litbi_ignorelist.join());
		...
		newElement.innerHTML = "clear lit Better Ignore cache";
		newElement.onclick = function() {
			var len = localStorage.length;
			for(var i=len - 1; i>=0; i--) {
				var key = localStorage.key(i);
				GM_log( i + " " + key + " " + key.indexOf('litbi_ignoredposts.'));
				if(key.indexOf('litbi_ignoredposts.') == 0) {
					localStorage.removeItem(key);
				}
			}
			window.alert("lit Better Ignore cache cleared.");
		}
		...
		try {
			var litbi_ignorelist_a = localStorage.getItem("litbi_ignorelist").split(',');
			for(var i in litbi_ignorelist_a) {
				litbi_ignorelist[litbi_ignorelist_a[i]] = true;
			}
		} catch(e) {
		}
		...
		if(window.location.pathname == "/forumdisplay.php") {
			...
			for(var i=0; i<xpathResult.snapshotLength; i++) {
				...
				if(rx && litbi_ignorelist[rx[1]]) {
					thisItem.parentNode.parentNode.parentNode.style.display = "none";
				}
			}
		} else if(window.location.pathname == "/showthread.php") {
...
//			thread_id = (window.location.search.split(/=|&/))[1];
			...
			try {
				ignored_posts_a = localStorage.getItem("litbi_ignoredposts." + thread_id).split(',');
			...
			for(var i=0; i<postTables.snapshotLength;i++) {
				...
				thisPost.className += " litbi_ignored";
				thisPost.style.display = "none"; // FIXME: remove when css for litbi_ignored is inserted
			...
			if(ignored_posts_a.length > 2) {
				localStorage.setItem("litbi_ignoredposts." + thread_id, ignored_posts_a);
			}
		}
	}
})();
